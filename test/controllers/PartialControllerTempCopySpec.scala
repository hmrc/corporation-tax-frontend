...
class PartialControllerTempCopySpec extends ControllerSpecBase with MockitoSugar {
  ...
  trait LocalSetup {
    ...
    lazy val c: PartialControllerTempCopy = new PartialControllerTempCopy(messagesApi, FakeAuthAction, FakeServiceInfoAction,
  ...
  "Calling PartialController.getCard" must {
    "return 200 in json format when asked to get a card and the call to the backend succeeds" in new LocalSetup {
      ...
      val result: Future[Result] = c.getCardTempAlternate(fakeRequest)
