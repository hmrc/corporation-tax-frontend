...
class SubpageControllerSpec extends ControllerSpecBase with MockitoSugar with ScalaFutures with ViewSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new SubpageController(frontendAppConfig, messagesApi, FakeAuthAction(cc.parsers.defaultBodyParser), FakeServiceInfoAction,
      mockAccountSummaryHelper, cc, subpage)
