...
trait Target {
  ...
  def toAttr: String = LinkHelper.attr("target", targetName)
...
trait PossibleSso {
  ...
  def toAttr: String = LinkHelper.attr("data-sso", value)
}
...
case class Link(url: String,
  ...
  private def hrefAttr = LinkHelper.attr("href", url)
  private def idAttr = id.map(LinkHelper.attr("id", _)).getOrElse("")

  private def text = value.map(v => Messages(v)).getOrElse("")
  private def cssAttr = cssClasses.map(LinkHelper.attr("class", _)).getOrElse("")
  ...
  private def relAttr = if(target == NewWindow) LinkHelper.attr("rel", "external noopener noreferrer") else ""
